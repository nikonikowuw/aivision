...
> 所有新增或修改的业务代码、公共接口、平台适配与算法包工程，**必须包含完备的中文注释**。无注释或注释残缺的代码严禁合并。

### 3.1 文件头注释
每个源码文件（`.hpp`、`.h`、`.cpp`、`.mm`、`.c`）头部必须包含 Doxygen 标准文件头：
- `@file`：文件名；
- `@brief`：核心职责与功能简述；
- 复杂模块应补充架构设计要点、调用流程或生命周期注意事项。

### 3.2 接口、类与结构体注释
- **公共接口与类**：所有抽象基类（如 `IDecoder`、`IImageProcessor`、`IMediaSource`、`ITelemetry`）必须注释其生命周期、线程模型与所属层级；
- **C ABI 结构体与函数指针**：每个 ABI 结构体（`av_frame_desc`、`av_algo_abi` 等）及其内部字段必须注明字节布局、对齐要求、所有权归属与单位；
- **公共成员函数**：必须说明功能、参数前置条件、返回值语义及可能产生的错误状态码。

### 3.3 关键业务逻辑行内注释
以下场景必须在代码内部包含清晰的中文行内注释：
1. **状态机与重连流程**：如拉流状态流转（`CONNECTING -> RUNNING -> RECONNECTING`）、看门狗超时判定与指数退避延迟计算；
2. **视频与位流协议解析**：如 NALU 单元提取、Annex-B / AVCC 封装转换、SPS/VUI 色彩空间解析；
3. **并发、队列与锁控制**：多线程数据分发（如帧队列丢老帧策略、双互斥锁死锁规避、条件变量等待退出顺序）；
4. **内存与硬件资源边界**：平台底层 opaque 句柄（`CVPixelBufferRef` / `DMA-BUF`）的引用计数管理、级联释放回调及原子文件落盘流程；
5. **算力与准入算法**：资源账本点数超限判断、物理内存防 OOM 安全水位线计算。
