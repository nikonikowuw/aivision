...
/**
 * @file rules.hpp
 * @brief 车牌识别 ROI、Mask 与分界线规则的校验和运行时过滤
 */
...
inline float rule_cross(const av_point& a, const av_point& b, const av_point& c) {
    return (b.x - a.x) * (c.y - a.y) - (b.y - a.y) * (c.x - a.x);
}

inline bool rule_point_in_polygon(const av_point& point, const std::vector<av_point>& polygon) {
    bool inside = false;
...
inline bool rule_segments_intersect(const av_point& a, const av_point& b,
    ...
    const float d1 = rule_cross(c, d, a);
    const float d2 = rule_cross(c, d, b);
    const float d3 = rule_cross(a, b, c);
    const float d4 = rule_cross(a, b, d);
...
inline float rule_polygon_signed_area(const std::vector<av_point>& polygon) {
    float area = 0.0f;
...
inline bool validate_and_copy_rules(const av_rule* rules, uint32_t count,
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        if (rule.role != AV_RULE_LINE) {
            if (std::fabs(rule_polygon_signed_area(state.points)) <= 1e-6f) {
            ...
            for (size_t a = 0; a < state.points.size(); ++a) {
                ...
                for (size_t b = a + 1; b < state.points.size(); ++b) {
                    ...
                    if (a_next == b || b_next == a) continue;
                    if (rule_segments_intersect(state.points[a], state.points[a_next],
                                                 state.points[b], state.points[b_next])) {
...
inline bool rule_line_crossed(const av_point& previous, const av_point& current,
                              const av_point& line_a, const av_point& line_b, uint32_t direction) {
    const float previous_side = rule_cross(line_a, line_b, previous);
    const float current_side = rule_cross(line_a, line_b, current);
    if (std::fabs(previous_side) < 1e-6f || std::fabs(current_side) < 1e-6f ||
...
inline void filter_region_rules(std::vector<PlateObject>& objects, const std::vector<RuleState>& rules) {
    bool has_roi = false;
    for (const RuleState& rule : rules) has_roi = has_roi || rule.role == AV_RULE_ROI;
    if (!has_roi && std::none_of(rules.begin(), rules.end(), [](const RuleState& rule) {
            return rule.role == AV_RULE_MASK;
        })) {
        return;
    }

    std::vector<PlateObject> filtered;
    filtered.reserve(objects.size());
    for (const auto& object : objects) {
        const av_point point{
            (object.x_min + object.x_max) * 0.5f,
            object.y_max,
        };
        bool masked = false;
        bool in_roi = false;
        for (const RuleState& rule : rules) {
            if (rule.role == AV_RULE_MASK && rule_point_in_polygon(point, rule.points)) masked = true;
            if (rule.role == AV_RULE_ROI && rule_point_in_polygon(point, rule.points)) in_roi = true;
        }
        if (!masked && (!has_roi || in_roi)) filtered.push_back(object);
    }
    objects = std::move(filtered);
}

inline void filter_line_rules(std::vector<PlateObject>& objects, const std::vector<RuleState>& rules,
                              std::unordered_map<int64_t, av_point>& previous_points,
                              std::unordered_map<int64_t, uint32_t>& missed_frames) {
    const bool has_line = std::any_of(rules.begin(), rules.end(), [](const RuleState& rule) {
        return rule.role == AV_RULE_LINE;
    });
    if (!has_line) return;
    ...
    std::vector<PlateObject> crossed_objects;
    crossed_objects.reserve(objects.size());
    for (auto& object : objects) {
        ...
        if (object.track_id >= 0) {
            ...
            if (previous != previous_points.end()) {
                for (const RuleState& rule : rules) {
                    if (rule.role != AV_RULE_LINE) continue;
                    for (size_t i = 1; i < rule.points.size(); ++i) {
                        if (rule_line_crossed(previous->second, point, rule.points[i - 1],
                                              rule.points[i], rule.mode)) {
                            crossed = true;
                            break;
                        }
                    }
        ...
        if (crossed) crossed_objects.push_back(std::move(object));
    }
    objects = std::move(crossed_objects);
}
