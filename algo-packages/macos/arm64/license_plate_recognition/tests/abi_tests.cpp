...
#ifndef LICENSE_PLATE_RECOGNITION_PACKAGE_ROOT
#define LICENSE_PLATE_RECOGNITION_PACKAGE_ROOT "."
#endif

int main() {
    const char* env_path = std::getenv("LICENSE_PLATE_RECOGNITION_DYLIB");
    std::string dylib_path = env_path ? env_path : "";
#ifdef LICENSE_PLATE_RECOGNITION_DYLIB_TARGET
    if (dylib_path.empty()) dylib_path = LICENSE_PLATE_RECOGNITION_DYLIB_TARGET;
#endif
    if (dylib_path.empty()) dylib_path = "lib/liblicense_plate_recognition.dylib";

    void* handle = dlopen(dylib_path.c_str(), RTLD_NOW | RTLD_LOCAL);
    if (!handle) {
        dylib_path = "lib/liblicense_plate_recognition.dylib";
    ...
    lib_args.package_root = LICENSE_PLATE_RECOGNITION_PACKAGE_ROOT;
    ...
    assert(st == AV_OK);
    assert(std::string(info.algorithm_id) == "license_plate_recognition");
    assert(std::string(info.algorithm_type) == "license_plate_recognition");
    assert(std::string(info.alarm_type_id) == ""); // Observation record: must be empty

    abi->library_close(lib);
    dlclose(handle);

    std::cout << "[PASS] license_plate_recognition ABI and metadata tests" << std::endl;
    return 0;
}
