...
int main() {
    ...
    const fs::path root = fs::temp_directory_path() / ("argus-yolo26n-loader-" + std::to_string(suffix));
    ...
    std::string error;
    require_condition(yolo26n::resolve_manifest_model_path(root.string(), model_path, error));
