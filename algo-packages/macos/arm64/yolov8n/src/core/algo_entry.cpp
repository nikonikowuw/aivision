...
struct LibraryContext {
    ...
    std::shared_ptr<CoreMLRunner> model_runner;
};
...
bool validate_frame(const av_frame_desc* frame) {
    ...
    if (!frame->opaque || frame->opaque_kind != AV_OPAQUE_CVPIXELBUFFER ||
        frame->memory_type != AV_MEM_PLATFORM_SURFACE || frame->pixel_format != AV_PIX_NV12 ||
        frame->layout != AV_LAYOUT_PLATFORM_NATIVE || frame->plane_count != 2) {
        return false;
    }
...
bool run_pipeline(InstanceContext* inst, const av_frame_desc* frame,
    ...
    struct PixelBufferGuard {
        void* value;
        ~PixelBufferGuard() { Preprocessor::release_pixelbuffer(value); }
    } guard{input_pixelbuffer};

    std::vector<float> network_output;
    if (!inst->lib->model_runner->run_pixelbuffer(input_pixelbuffer, network_output)) {
        set_error(inst, "Core ML inference execution failed");
        return false;
    }

    const auto raw_objects = Postprocessor::postprocess(
        network_output,
...
int yolo_instance_negotiate_impl(av_algo_instance inst_handle, const av_frame_caps* offered, av_frame_caps* accepted) {
    ...
    if (offered->required_opaque_kind != AV_OPAQUE_NONE && offered->required_opaque_kind != AV_OPAQUE_CVPIXELBUFFER) {
    ...
    if (!contains(offered->pixel_formats, offered->pixel_format_count, AV_PIX_NV12) ||
        !contains(offered->memory_types, offered->memory_type_count, AV_MEM_PLATFORM_SURFACE)) {
        return fail(static_cast<InstanceContext*>(inst_handle), AV_ERR_INCOMPATIBLE_FRAME, "offered frame has no NV12 platform-surface option");
    }
    ...
    *accepted = {};
    ...
    accepted->memory_type_count = 1;
    accepted->memory_types[0] = AV_MEM_PLATFORM_SURFACE;
    accepted->required_opaque_kind = AV_OPAQUE_CVPIXELBUFFER;
