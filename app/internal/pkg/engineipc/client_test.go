...
func startFakeEngine(t *testing.T, fake *fakeEngineServiceServer) string {
	...
	srv := grpc.NewServer()
	aivisionv1.RegisterEngineServiceServer(srv, fake)
...
func TestEngineClientAllRPCs(t *testing.T) {
	...
	// 1. ApplyDesiredState
	resp1, err := client.ApplyDesiredState(ctx, &aivisionv1.ApplyDesiredStateRequest{
		DesiredState: &aivisionv1.DesiredState{DeviceId: "dev-1", Revision: 9},
	})
	if err != nil {
	...
	// 2. UpsertTask
	resp2, err := client.UpsertTask(ctx, &aivisionv1.UpsertTaskRequest{
		Task: &aivisionv1.CameraTaskConfig{CameraId: "cam-1", RtspUrl: "rtsp://x", Enabled: true},
	})
	...
	// 3. SetInstanceState
	resp3, err := client.SetInstanceState(ctx, &aivisionv1.SetInstanceStateRequest{InstanceId: "inst-1", Enabled: false})
	if err != nil || resp3.GetCode() != CodeOK {
		t.Fatalf("SetInstanceState: %v resp=%+v", err, resp3)
	}

	// 4. UpdateInstanceConfig
	resp4, err := client.UpdateInstanceConfig(ctx, &aivisionv1.UpdateInstanceConfigRequest{InstanceId: "inst-1", ParamsJson: `{"x":1}`})
	if err != nil || resp4.GetCode() != CodeOK {
		t.Fatalf("UpdateInstanceConfig: %v resp=%+v", err, resp4)
	}

	// 5. InstallPackage
	resp5, err := client.InstallPackage(ctx, &aivisionv1.InstallPackageRequest{PackagePath: "/tmp/pkg.tar.gz"})
	if err != nil || resp5.GetCode() != CodeOK || resp5.GetAlgorithmId() != "yolov8n" {
		t.Fatalf("InstallPackage: %v resp=%+v", err, resp5)
	}

	// 6. UpgradePackage
	resp6, err := client.UpgradePackage(ctx, &aivisionv1.UpgradePackageRequest{PackagePath: "/tmp/pkg2.tar.gz"})
	if err != nil || resp6.GetCode() != CodeOK || resp6.GetVersion() != "1.1.0" {
		t.Fatalf("UpgradePackage: %v resp=%+v", err, resp6)
	}

	// 7. RollbackPackage
	resp7, err := client.RollbackPackage(ctx, &aivisionv1.RollbackPackageRequest{AlgorithmId: "yolov8n", TargetVersion: "1.0.0"})
	if err != nil || resp7.GetCode() != CodeOK {
		t.Fatalf("RollbackPackage: %v resp=%+v", err, resp7)
	}

	// 8. UninstallPackage
	resp8, err := client.UninstallPackage(ctx, &aivisionv1.UninstallPackageRequest{AlgorithmId: "yolov8n", Version: "1.0.0"})
	if err != nil || resp8.GetCode() != CodeOK {
		t.Fatalf("UninstallPackage: %v resp=%+v", err, resp8)
	}

	// 9. DeleteImages
	resp9, err := client.DeleteImages(ctx, &aivisionv1.DeleteImagesRequest{ImageIds: []string{"a", "b"}})
	...
	// 10. ReconcileImages
	resp10, err := client.ReconcileImages(ctx, &aivisionv1.ReconcileImagesRequest{RetainImageIds: []string{"a"}})
	if err != nil || resp10.GetCode() != CodeOK {
		t.Fatalf("ReconcileImages: %v resp=%+v", err, resp10)
	}

	// 11. QueryProfile
	resp11, err := client.QueryProfile(ctx, &aivisionv1.QueryProfileRequest{})
	if err != nil || resp11.GetCode() != CodeOK || resp11.GetProfile().GetPlatformId() != "fake" {
		t.Fatalf("QueryProfile: %v resp=%+v", err, resp11)
	}

	// 12. QueryMetrics
	resp12, err := client.QueryMetrics(ctx, &aivisionv1.QueryMetricsRequest{})
...
func TestEngineClientRemoteError(t *testing.T) {
	...
	resp, err := client.ApplyDesiredState(clientCtx(t), &aivisionv1.ApplyDesiredStateRequest{})
	if err == nil {
...
func TestEngineClientTransportOKForBusinessError(t *testing.T) {
	...
	_, err := client.InstallPackage(clientCtx(t), &aivisionv1.InstallPackageRequest{})
	if err == nil {
...
func TestEngineClientEngineAbsent(t *testing.T) {
	...
	defer cancel()
	_, err := client.QueryProfile(ctx, &aivisionv1.QueryProfileRequest{})
	if err == nil {
...
func TestEngineClientReconnectAfterServerAppears(t *testing.T) {
	...
	var resp *aivisionv1.QueryProfileResponse
	var lastErr error
	for i := 0; i < 20; i++ {
		ctx, cancel := context.WithTimeout(context.Background(), 500*time.Millisecond)
		resp, lastErr = client.QueryProfile(ctx, &aivisionv1.QueryProfileRequest{})
		cancel()
		if lastErr == nil {
			break
		}
...
func TestEngineClientServerRestart(t *testing.T) {
	...
	srv := startSrv()
	client := newTestEngineClient(t, path)
	if _, err := client.QueryProfile(clientCtx(t), &aivisionv1.QueryProfileRequest{}); err != nil {
	...
	for i := 0; i < 20; i++ {
		ctx, cancel := context.WithTimeout(context.Background(), 500*time.Millisecond)
		_, lastErr = client.QueryProfile(ctx, &aivisionv1.QueryProfileRequest{})
		cancel()
		if lastErr == nil {
			break
		}
...
func TestEngineClientClose(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 300*time.Millisecond)
	defer cancel()
	if _, err := client.QueryProfile(ctx, &aivisionv1.QueryProfileRequest{}); err == nil {
