...
type controlPlaneService struct {
	aivisionv1.UnimplementedControlPlaneServiceServer
	adapter DesiredStateAdapter
	log     *zap.Logger
}

// GetDesiredState 返回 Go 权威的期望状态；adapter 错误归一化为稳定响应 code。
func (s *controlPlaneService) GetDesiredState(ctx context.Context, req *aivisionv1.GetDesiredStateRequest) (*aivisionv1.GetDesiredStateResponse, error) {
	if req == nil {
	...
	if err != nil {
		...
		if code == CodeInternalError {
			s.log.Warn("desired state adapter failed",
				zap.String("method", "/aivision.v1.ControlPlaneService/GetDesiredState"),
				zap.Error(err))
		}
		return &aivisionv1.GetDesiredStateResponse{Code: code, ErrorMessage: msg}, nil
	}
	if ds == nil {
		// 内部实现错误：adapter 成功但未返回状态，绝不伪装成成功。
		s.log.Error("desired state adapter returned nil state",
			zap.String("method", "/aivision.v1.ControlPlaneService/GetDesiredState"))
		return &aivisionv1.GetDesiredStateResponse{
			Code:         CodeInternalError,
			ErrorMessage: "internal error",
		}, nil
	}
	return &aivisionv1.GetDesiredStateResponse{DesiredState: ds, Code: CodeOK}, nil
}

// reportService 实现生成的 ReportServiceServer：Engine 通过 app.sock 异步上报
// 告警、任务/实例状态、遥测与孤儿图片。
type reportService struct {
	aivisionv1.UnimplementedReportServiceServer
...
func (s *reportService) ReportAlarm(ctx context.Context, req *aivisionv1.ReportAlarmRequest) (*aivisionv1.ReportAlarmResponse, error) {
	const method = "/aivision.v1.ReportService/ReportAlarm"
	...
	return &aivisionv1.ReportAlarmResponse{Code: code, ErrorMessage: msg}, nil
}

// ReportTaskState 接收任务状态上报；只有 adapter 成功接受后才返回空 code。
func (s *reportService) ReportTaskState(ctx context.Context, req *aivisionv1.ReportTaskStateRequest) (*aivisionv1.ReportTaskStateResponse, error) {
	const method = "/aivision.v1.ReportService/ReportTaskState"
	...
	return &aivisionv1.ReportTaskStateResponse{Code: code, ErrorMessage: msg}, nil
}

// ReportInstanceState 接收算法实例状态上报；transport 错误不转换为业务响应。
func (s *reportService) ReportInstanceState(ctx context.Context, req *aivisionv1.ReportInstanceStateRequest) (*aivisionv1.ReportInstanceStateResponse, error) {
	const method = "/aivision.v1.ReportService/ReportInstanceState"
	...
	return &aivisionv1.ReportInstanceStateResponse{Code: code, ErrorMessage: msg}, nil
}

// ReportMetrics 接收设备遥测上报；普通 adapter 错误只暴露受控 code。
func (s *reportService) ReportMetrics(ctx context.Context, req *aivisionv1.ReportMetricsRequest) (*aivisionv1.ReportMetricsResponse, error) {
	const method = "/aivision.v1.ReportService/ReportMetrics"
	...
	return &aivisionv1.ReportMetricsResponse{Code: code, ErrorMessage: msg}, nil
}

// ReportOrphanImages 接收孤儿图片对账请求；失败时不得返回空 code 使 Engine 删除图片。
func (s *reportService) ReportOrphanImages(ctx context.Context, req *aivisionv1.ReportOrphanImagesRequest) (*aivisionv1.ReportOrphanImagesResponse, error) {
	const method = "/aivision.v1.ReportService/ReportOrphanImages"
	if req == nil {
		return nil, invalidArgument(method)
	}
	...
	return &aivisionv1.ReportOrphanImagesResponse{
