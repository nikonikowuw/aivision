...
// 与 18 个本任务 RPC（6 个 Go server 入站 + 12 个 Engine client 出站）。
package aivisionv1
...
func TestGeneratedDescriptorMatchesProto(t *testing.T) {
	...
	// 本任务范围：6 个入站（ControlPlane 1 + Report 5）+ 12 个出站 EngineService = 18。
	// PersonService.SyncPersons 为预留接口，不计入本任务 18 个 RPC。
	taskRPCs := 0
	...
	if taskRPCs != 18 {
		t.Errorf("in-scope RPC count = %d, want 18", taskRPCs)
	}
}
