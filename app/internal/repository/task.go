...
func (r *taskRepository) LoadDesiredSnapshot(ctx context.Context) (*aivisionv1.DesiredState, error) {
	...
	state := &aivisionv1.DesiredState{
		Tasks:                 make([]*aivisionv1.CameraTaskConfig, 0, len(taskRows)),
		Instances:             make([]*aivisionv1.AlgorithmInstanceConfig, 0, len(instanceRows)),
		ActivePackageVersions: make([]*aivisionv1.ActivePackageVersion, 0, len(versionRows)),
	}
	for _, row := range taskRows {
		enabledCameras[row.CameraID] = struct{}{}
		state.Tasks = append(state.Tasks, &aivisionv1.CameraTaskConfig{
	...
	for _, row := range instanceRows {
		...
		state.Instances = append(state.Instances, &aivisionv1.AlgorithmInstanceConfig{
	...
	for _, row := range versionRows {
		state.ActivePackageVersions = append(state.ActivePackageVersions, &aivisionv1.ActivePackageVersion{
			AlgorithmId: row.AlgorithmID,
...
func parseRulesJSON(instanceID string, raw json.RawMessage) ([]*aivisionv1.DetectionRule, error) {
	if len(raw) == 0 {
		return nil, nil
	}
...
func toProtoRules(rules []model.DetectionRule) []*aivisionv1.DetectionRule {
	out := make([]*aivisionv1.DetectionRule, 0, len(rules))
	for _, r := range rules {
		pr := &aivisionv1.DetectionRule{
			Role:          aivisionv1.DetectionRuleRole(r.Role),
			LineDirection: aivisionv1.DetectionLineDirection(r.LineDirection),
			Points:        make([]*aivisionv1.DetectionPoint, 0, len(r.Points)),
		}
		for _, p := range r.Points {
			pr.Points = append(pr.Points, &aivisionv1.DetectionPoint{X: float32(p.X), Y: float32(p.Y)})
		}
		out = append(out, pr)
	}
	return out
