...
func (f *fakeProfileClient) set(profile *aivisionv1.QueryProfileResponse, err error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.profile = profile
	f.err = err
...
func profileResp(total, reserved int32) *aivisionv1.QueryProfileResponse {
...
func TestTaskServiceStatusMerge(t *testing.T) {
	...
	if err := report.AcceptInstanceState(ctx, &aivisionv1.InstanceState{
	...
	if err := report.AcceptTaskState(ctx, &aivisionv1.TaskState{
		CameraId:            "cam-a",
