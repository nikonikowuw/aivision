...
#
# 用法：
#   ./scripts/generate-proto.sh                 # 输出到 app/internal/proto/aivision/v1（提交目录）
#   ./scripts/generate-proto.sh <out-root>      # 输出到指定模块根（供 proto-check 漂移检查）
#
# 约定：
#   - 唯一 proto 权威源是 engine/proto/aivision/v1/*.proto，本脚本只读取该目录。
#   - go_package 为 niko-vue-admin/app/internal/proto/aivision/v1，配合
#     --go_opt=module=niko-vue-admin/app 生成文件落在 <out-root>/internal/proto/aivision/v1/。
...
PROTO_SRC="$APP_DIR/../engine/proto/aivision/v1"
REL_OUT="internal/proto/aivision/v1"
...
"$PROTOC" \
  ...
  --go_out="$OUT_ROOT" \
  --go_opt=module=niko-vue-admin/app \
  --go-grpc_out="$OUT_ROOT" \
  --go-grpc_opt=module=niko-vue-admin/app \
  "${PROTOS[@]}"
