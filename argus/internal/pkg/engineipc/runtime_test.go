...
func TestRuntimeLifecycle(t *testing.T) {
	...
	defer cancel()
	if _, err := cp.GetDesiredState(ctx, &argusv1.GetDesiredStateRequest{}); err != nil {
...
func (a *blockingDesiredStateAdapter) DesiredState(ctx context.Context, _ uint64) (*argusv1.DesiredState, error) {
	select {
	...
	select {
	case <-a.release:
		return &argusv1.DesiredState{}, nil
	case <-ctx.Done():
		return nil, ctx.Err()
	}
}
...
func TestRuntimeForceStop(t *testing.T) {
	...
	go func() {
		_, err := cp.GetDesiredState(rpcCtx, &argusv1.GetDesiredStateRequest{})
		rpcErr <- err
	}()
	select {
...
func TestRuntimeShutdownStopsNewCalls(t *testing.T) {
	...
	defer callCancel()
	if _, err := cp.GetDesiredState(callCtx, &argusv1.GetDesiredStateRequest{}); err == nil {
...
func TestRuntimeConcurrentShutdown(t *testing.T) {
	adapter := &recordingDesiredStateAdapter{state: &argusv1.DesiredState{Revision: 1}}
	path := testSocketPath(t, "app.sock")
	rt := NewRuntime(zap.NewNop(), adapter, &recordingReportAdapter{})
	if err := rt.Start(path); err != nil {
		t.Fatalf("Start: %v", err)
	...
	for i := 0; i < 8; i++ {
		...
		go func() {
			...
			for {
				...
				_, _ = cp.GetDesiredState(ctx, &argusv1.GetDesiredStateRequest{})
				cancel()
			}
		}()
	}
