...
func TestReportAdapterSerializesTaskUpdates(t *testing.T) {
	...
	go func() {
		firstDone <- adapter.AcceptTaskState(ctx, &argusv1.TaskState{
	...
	go func() {
		secondDone <- adapter.AcceptTaskState(ctx, &argusv1.TaskState{
			CameraId: "cam-concurrent",
			Status:   argusv1.TaskStatusCode_TASK_STATUS_STOPPED,
		})
	}()
	...
	if len(statuses) != 2 || statuses[0] != int8(argusv1.TaskStatusCode_TASK_STATUS_RUNNING) || statuses[1] != int8(argusv1.TaskStatusCode_TASK_STATUS_STOPPED) {
		t.Fatalf("task persistence order = %v, want [RUNNING STOPPED]", statuses)
	}
	rt, ok := adapter.TaskRuntime("cam-concurrent")
	if !ok || rt.Status != int8(argusv1.TaskStatusCode_TASK_STATUS_STOPPED) {
...
func TestReportAdapterSerializesInstanceUpdates(t *testing.T) {
	...
	firstDone := make(chan error, 1)
	go func() {
		firstDone <- adapter.AcceptInstanceState(ctx, &argusv1.InstanceState{
	...
	secondDone := make(chan error, 1)
	go func() {
		secondDone <- adapter.AcceptInstanceState(ctx, &argusv1.InstanceState{
			InstanceId: "inst-concurrent",
			Status:     argusv1.InstanceStatusCode_INSTANCE_STATUS_STOPPED,
		})
	}()
	...
	if len(statuses) != 2 || statuses[0] != int8(argusv1.InstanceStatusCode_INSTANCE_STATUS_RUNNING) || statuses[1] != int8(argusv1.InstanceStatusCode_INSTANCE_STATUS_STOPPED) {
		t.Fatalf("instance persistence order = %v, want [RUNNING STOPPED]", statuses)
	}
	rt, ok := adapter.InstanceRuntime("inst-concurrent")
	if !ok || rt.Status != int8(argusv1.InstanceStatusCode_INSTANCE_STATUS_STOPPED) {
...
func TestReportAdapterWritesOnlyOnStatusChange(t *testing.T) {
	...
	if err := adapter.AcceptInstanceState(ctx, &argusv1.InstanceState{
		InstanceId: "i1", Status: argusv1.InstanceStatusCode_INSTANCE_STATUS_RUNNING,
	...
	if err := adapter.AcceptInstanceState(ctx, &argusv1.InstanceState{
		InstanceId: "i1", Status: argusv1.InstanceStatusCode_INSTANCE_STATUS_STOPPED,
		Message: "stopped", CurrentFps: 0,
	}); err != nil {
		t.Fatalf("third accept: %v", err)
	}
	// 任务状态：STARTING → STARTING（同码）。
	if err := adapter.AcceptTaskState(ctx, &argusv1.TaskState{
		CameraId: "cam-a", Status: argusv1.TaskStatusCode_TASK_STATUS_STARTING,
	...
	if rt.Status != int8(argusv1.InstanceStatusCode_INSTANCE_STATUS_STOPPED) || rt.CurrentFps != 0 {
...
func TestReportAdapterStatusPersisted(t *testing.T) {
	...
	adapter := NewReportAdapter(repo, zap.NewNop())
	if err := adapter.AcceptTaskState(ctx, &argusv1.TaskState{
		CameraId: "cam-a", Status: argusv1.TaskStatusCode_TASK_STATUS_RUNNING, Message: "running",
	}); err != nil {
		t.Fatalf("accept task state: %v", err)
	}
	if err := adapter.AcceptInstanceState(ctx, &argusv1.InstanceState{
		InstanceId: "i1", Status: argusv1.InstanceStatusCode_INSTANCE_STATUS_ERROR, Message: "oom",
	}); err != nil {
...
func TestReportAdapterUnimplementedFailClosed(t *testing.T) {
	...
	if err := adapter.AcceptAlarm(context.Background(), &argusv1.AlarmEvent{}); !isAdapterUnavailable(err) {
		t.Errorf("AcceptAlarm err = %v, want IPC_UNAVAILABLE", err)
	}
	if err := adapter.AcceptMetrics(context.Background(), &argusv1.DeviceTelemetry{}); !isAdapterUnavailable(err) {
...
func TestReportAdapterRejectsEmptyIDs(t *testing.T) {
	...
	if err := adapter.AcceptTaskState(ctx, &argusv1.TaskState{}); err == nil {
		t.Error("empty camera_id task state unexpectedly accepted")
	}
	if err := adapter.AcceptInstanceState(ctx, &argusv1.InstanceState{}); err == nil {
