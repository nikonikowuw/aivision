-- 000033_seed_record_face_menu.up.sql
-- 幂等写入人脸识别记录 (RecordFace) 菜单及权限按钮。

DO $$
DECLARE
    v_super_role_id BIGINT;
    v_record_id BIGINT;
    v_face_menu_id BIGINT;
    v_btn_id BIGINT;
    r_btn RECORD;
BEGIN
    ...
    SELECT id INTO v_face_menu_id FROM menus WHERE parent_id = v_record_id AND name = 'RecordFace' LIMIT 1;
    IF v_face_menu_id IS NULL THEN
        INSERT INTO menus (
            parent_id, type, name, title, path, component, icon, sort, status,
            permission, affix, keep_alive, home_path, created_at, updated_at, deleted_at
        ) VALUES (
            v_record_id, 'menu', 'RecordFace', 'routes.record.face', '/record/face', '/record/face/index', 'ant-design:user-outlined', 3, 1,
            'record:face', FALSE, TRUE, '', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 0
        ) RETURNING id INTO v_face_menu_id;
    END IF;
    IF v_super_role_id IS NOT NULL THEN
        INSERT INTO role_menus (role_id, menu_id) VALUES (v_super_role_id, v_face_menu_id) ON CONFLICT (role_id, menu_id) DO NOTHING;
    END IF;

    FOR r_btn IN SELECT * FROM (VALUES
        ('record.face.query',  'record:face:query',  1),
        ('record.face.export', 'record:face:export', 2)
    ) AS t(name, perm, sort_order)
    LOOP
        v_btn_id := NULL;
        SELECT id INTO v_btn_id
        FROM menus
        WHERE parent_id = v_face_menu_id AND permission = r_btn.perm
        LIMIT 1;
        ...
        IF v_btn_id IS NULL THEN
            INSERT INTO menus (
                ...
                v_face_menu_id, 'button', r_btn.name, r_btn.name, '', '', '', r_btn.sort_order, 1,
