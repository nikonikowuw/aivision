...
func (a *e2eDesiredStateAdapter) DesiredState(context.Context, uint64) (*argusv1.DesiredState, error) {
	a.mu.Lock()
	a.calls++
	a.mu.Unlock()
	select {
...
func (a *e2eReportAdapter) AcceptAlarm(context.Context, *argusv1.AlarmEvent) error {
	return nil
}

func (a *e2eReportAdapter) AcceptTaskState(context.Context, *argusv1.TaskState) error {
	return nil
}

func (a *e2eReportAdapter) AcceptInstanceState(context.Context, *argusv1.InstanceState) error {
	return nil
}

func (a *e2eReportAdapter) AcceptMetrics(context.Context, *argusv1.DeviceTelemetry) error {
	a.mu.Lock()
	a.metrics++
	a.mu.Unlock()
	select {
...
func (a *e2eReportAdapter) ReconcileOrphanImages(context.Context, []*argusv1.OrphanImageEntry) (engineipc.OrphanDisposition, error) {
...
func TestGrpcEngineE2E(t *testing.T) {
	...
	defer cancel()
	resp, err := client.QueryProfile(ctx, &argusv1.QueryProfileRequest{})
	if err != nil {
...
func TestGrpcEngineE2EValidation(t *testing.T) {
	...
	rt := engineipc.NewRuntime(zap.NewNop(),
		&e2eDesiredStateAdapter{onCall: make(chan struct{}, 1), state: &argusv1.DesiredState{DeviceId: "standalone", Revision: 1}},
	...
	cp := argusv1.NewControlPlaneServiceClient(conn)
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	if resp, err := cp.GetDesiredState(ctx, &argusv1.GetDesiredStateRequest{}); err != nil || resp.GetDesiredState() == nil {
