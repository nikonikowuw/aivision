...
| `object_detection` | 目标检测及算法内部事件判定 | 每帧最多一次批次回调，Engine 按目标生成告警记录 |
...
- 目标检测算法每帧回调一次检测批次，Engine 按批次中的每个目标生成一条独立告警记录；同一批次的告警共享一张抓拍图片。
...
- [ ] 一个批次中的每个目标生成独立告警记录，目标事件 ID 不重复且同批次告警共享一张图片。
- [ ] 重复 `event_id` 不重复落库。
- [ ] 三类记录均可按约定条件分页查询。
- [ ] 不保存事件视频，不提供告警处置状态机。
