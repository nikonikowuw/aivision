...
| 版本 | 日期 | 修订内容 | 状态 |
| --- | --- | --- | --- |
| V1.0 | 2026-08-22 | 根据需求访谈形成首版 PRD | 待评审 |
...
| C++ 推理引擎 | 负责 ZLMediaKit、视频接入、硬件解码、共享帧、算法调度、RKNN 推理及图片处理的原生服务 |
| 算法包 | 遵循平台 SDK 和 C ABI 规范，面向一个明确硬件平台发布的模型、动态库、清单、配置 Schema 和测试图片集合 |
...
| NPU 资源单位 | 平台用于启用前容量校验的抽象算力配额，RK3576 整机总容量定义为 1000 单位 |
| 高低水位 | 磁盘容量清理的启动阈值和停止阈值 |
| ZLM | ZLMediaKit，嵌入 C++ 推理引擎并统一负责摄像头拉流和视频预览媒体源 |
...
本产品部署在企业局域网内的 RK3576 边缘设备上，将摄像头视频转换为抓拍、人员识别和安全告警记录，并通过可配置 Webhook 推送给业务系统。平台重点解决视频接入、算法包标准化、推理任务调度、结果持久化和业务集成问题，不承担考勤规则或具体安全算法的研发。
...
1. 在 RK3576 上稳定接入并分析多路 1080p 网络视频。
2. 建立支持人脸识别和目标检测两类算法的统一算法包规范。
3. 支持一个摄像头任务挂载多个算法，并共享拉流和解码结果。
4. 保存抓拍、识别和告警记录，并可靠推送到一个或多个业务平台。
5. 支持算法包独立升级、回滚、配置热更新和 NPU 资源配额校验。
6. 在本地设备资源受限时通过留存期和磁盘高低水位保证系统持续运行。
...
- 不提供批量记录导出。
- 不实现 CUDA、昇腾等其他推理后端，仅预留多平台 SDK 接口。
- 不要求算法包数字签名。
...
| C++ 媒体推理引擎 | 嵌入 ZLM，统一拉流和预览媒体源，MPP 解码，共享帧，算法包运行时，RKNN 调度、人脸质量检测与特征提取、内存索引、RGA 图像处理、硬件 JPEG 编码及全部图片文件生命周期 |
...
- Go 服务和 C++ 推理引擎为独立进程。
- ZLMediaKit 作为媒体库嵌入 C++ 推理引擎，不单独建立推理回环拉流。
- Go 与 C++ 通过 Unix Domain Socket 上的 gRPC/Protobuf 通信。
- 摄像头视频帧不得跨 Go/C++ 进程传输。
...
- 所有服务采用宿主机原生部署，由 systemd 管理开机启动、异常拉起、日志轮转和有序停止。

### 6.3 多平台接口要求

- SDK 的帧描述符、算法包清单和结果协议必须支持未来多硬件平台扩展。
- V1.0 仅实现 `RKNN/RK3576` 后端并完成验收。
- 通用接口不得暴露 MPP、RGA、RKNN 等平台私有结构。
- 平台相关资源通过不透明句柄和能力查询扩展。
- 一个算法包只能面向一个明确目标平台。
- 相同算法面向不同平台时分别构建和发布，但遵循相同 C ABI 和结果 Schema。
...
- ZLM 负责统一拉取摄像头视频。
- 推理和浏览器预览复用同一个 ZLM MediaSource。
- 不允许 ZLM 和推理模块分别建立摄像头上游 RTSP 连接。
- ZLM Track 回调仅允许转移帧引用到有界队列，不得执行解码、数据库、磁盘、Webhook 或推理等阻塞工作。
...
- 使用嵌入 C++ 推理引擎的 ZLMediaKit 提供实时视频预览。
- 具体浏览器输出协议以目标版本 ZLM 的编译能力和配置为准。
...
- 一个摄像头通道对应一个任务。
- 一个任务可以配置多个算法实例。
- 同一任务中的算法共享一次拉流和一次硬件解码结果。
- 每个算法实例具有独立的运行参数、采样 FPS、状态和 NPU 资源占用。
...
- C++ 推理引擎向算法提供共享的硬件原生解码帧，不固定为 NV12。
- 通用帧描述符至少包含硬件平台、内存类型、不透明平台句柄、像素格式、布局或压缩模式、有效宽高、分配宽高、平面 offset、stride、modifier、时间戳和 `frame_id`。
- 算法包声明支持的输入帧格式和内存类型，创建实例时进行能力协商。
- 不兼容时拒绝启用并返回明确原因。
- 每个算法按自身配置独立采样。
...
- 所有引用归零前不得复用底层帧缓冲。
- 算法不得关闭平台提供的 DMA-BUF fd 或长期保存无引用的裸指针。
- 算法超时后停止向该实例分发新帧，但不得强制复用仍可能被原生代码访问的缓冲。
...
- 同一 `algorithm_id + target_platform` 同一时间只允许一个激活版本。
- 可保留一个旧版本用于自动回滚。
...
- RK3576 可分配 NPU 容量抽象为 1000 个资源单位。
- 平台自身保留必要安全余量，不得将全部物理资源无条件分配给算法。
- 算法包根据目标板实测，按输入规格和 FPS 档位声明资源单位。
- 平台汇总所有启用算法实例的资源单位。
- 新增、启用或修改算法 FPS 后超过可分配容量时拒绝操作，并提示受限资源。
- 不允许根据模型文件大小推算 NPU 消耗。
- 内存不参与日常配额加和，但保留最低可用内存安全阈值；低于阈值时拒绝启动并回滚。
- 同一模型的并发工作单元使用独立推理上下文，并以共享模型权重为目标减少内存占用。
- 上下文复制是否实际共享设备权重不得仅根据 API 名称推断，必须在目标 RK3576、Runtime 和驱动组合上通过 `RKNN_QUERY_MEM_SIZE`、RSS/PSS 等实测确认。
...
- C++ 根据算法结果和共享帧完成裁剪、缩放、颜色转换、标注和 JPEG 编码。
- RGA 用于适用的裁剪、缩放和颜色转换；JPEG 编码使用目标 BSP 可用的硬件能力。
...
| NPU 配额不足 | 拒绝启用或 FPS 热更新，不影响现有实例 | 显示资源不足原因 |
...
| C++ 服务异常退出 | systemd 自动拉起，恢复启用任务 | 短时不可用后恢复 |
...
在固定 RK3576 板卡、BSP、Runtime、测试视频和模型工件下验收：
...
- 发布物必须固定并记录 RK3576 BSP、内核、RKNN Runtime、RKNPU 驱动、MPP、RGA 和 ZLM 版本组合。
- 算法包必须声明最低运行时版本和目标芯片。
- 共享权重、NPU 核绑定、帧格式和 DMA-BUF 行为均以目标板实测及已安装头文件为准。
...
1. Go 业务服务和管理前端。
2. C++ 媒体推理引擎。
3. 多平台可扩展的算法包 SDK 和 C ABI 头文件。
4. 算法包清单、配置 Schema、结果 Schema 和帧描述符规范。
5. Go/C++ gRPC/Protobuf 接口定义。
6. YOLOv8n RKNN 示例算法包及完整转换证据。
7. RK3576 原生安装包和 systemd 服务配置。
8. API 文档、算法包开发文档、部署文档和验收测试报告。

## 13. 风险与待冻结项

| 风险或待确认项 | 影响 | 冻结要求 |
| --- | --- | --- |
| RKNN 上下文复制的实际权重共享行为未经目标板证明 | 影响 4～16 路场景内存容量 | 在固定 Runtime/驱动和模型上测量独立初始化与上下文复制的 RSS/PSS、查询内存和稳定性 |
...
| NPU 资源单位与 FPS 档位尚无实测表 | 影响超配判断准确性 | 在固定 RK3576 环境形成算法包资源基线并版本化 |
