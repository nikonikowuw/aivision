...
bool UdsClient::report_alarm(const aivision::v1::AlarmEvent& alarm) {
    if (!report_stub_) return false;
    aivision::v1::ReportAlarmRequest req;
...
bool UdsClient::report_telemetry(const aivision::v1::DeviceTelemetry& telemetry) {
    if (!report_stub_) return false;
    aivision::v1::ReportMetricsRequest req;
    *req.mutable_telemetry() = telemetry;
    aivision::v1::ReportMetricsResponse resp;
    grpc::ClientContext ctx;
    ctx.set_deadline(std::chrono::system_clock::now() + std::chrono::seconds(2));
...
bool UdsClient::report_task_state(const aivision::v1::TaskState& task_state) {
    if (!report_stub_) return false;
    aivision::v1::ReportTaskStateRequest request;
    *request.mutable_task_state() = task_state;
    aivision::v1::ReportTaskStateResponse response;
...
bool UdsClient::report_instance_state(const aivision::v1::InstanceState& instance_state) {
    if (!report_stub_) return false;
    aivision::v1::ReportInstanceStateRequest request;
    *request.mutable_instance_state() = instance_state;
    aivision::v1::ReportInstanceStateResponse response;
    grpc::ClientContext context;
    context.set_deadline(std::chrono::system_clock::now() + std::chrono::seconds(2));
...
bool UdsClient::report_orphan_images(const std::vector<aivision::v1::OrphanImageEntry>& orphan_images,
                                     aivision::v1::ReportOrphanImagesResponse* out_response) {
    if (!report_stub_ || !out_response) return false;
    aivision::v1::ReportOrphanImagesRequest request;
...
bool UdsClient::get_desired_state(uint64_t current_revision, aivision::v1::DesiredState* out_state) {
    if (!control_plane_stub_ || !out_state) return false;
    aivision::v1::GetDesiredStateRequest req;
    req.set_current_revision(current_revision);
    aivision::v1::GetDesiredStateResponse resp;
    grpc::ClientContext ctx;
    ctx.set_deadline(std::chrono::system_clock::now() + std::chrono::seconds(2));
