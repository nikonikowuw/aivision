        ...
        bool accepted = false;
        {
            ...
            for (const auto& candidate : candidates) {
                ...
                const bool due = state == generic_track_states_.end() ||
                                 now - state->second.last_report_at >= std::chrono::milliseconds(3000) ||
                ...
                if (candidate.target_type == "person" && candidate.has_face) {
                    const float fx_min = std::clamp(static_cast<float>(candidate.face_bbox[0]), 0.0f, 0.9999f);
                    const float fy_min = std::clamp(static_cast<float>(candidate.face_bbox[1]), 0.0f, 0.9999f);
                    const float fx_max = std::clamp(static_cast<float>(candidate.face_bbox[0] + candidate.face_bbox[2]), fx_min + 1e-4f, 1.0f);
                    const float fy_max = std::clamp(static_cast<float>(candidate.face_bbox[1] + candidate.face_bbox[3]), fy_min + 1e-4f, 1.0f);
                    event.mutable_sub_bbox()->set_x_min(fx_min);
                    event.mutable_sub_bbox()->set_y_min(fy_min);
                    event.mutable_sub_bbox()->set_x_max(fx_max);
                    event.mutable_sub_bbox()->set_y_max(fy_max);
                }
                ...
                if (candidate.target_type == "face") {
                    crop_roi = expand_face_crop_roi(candidate.bbox.data(), 0.4f);
                } else {
                ...
                if (candidate.target_type == "person" && candidate.has_face) {
                    sub_crop_roi = expand_face_crop_roi(candidate.face_bbox.data(), 0.4f);
                }
