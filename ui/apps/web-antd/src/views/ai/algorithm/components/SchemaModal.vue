...
import { Button, Empty, Modal, Table, Tabs, Tag, Tooltip } from 'ant-design-vue';
...
const dataSource = computed<SchemaPropertyRow[]>(() => {
  ...
  return Object.keys(properties).map((key) => {
    ...
    const baseDesc =
      formatPropertyDesc(key, rawDesc) || rawDesc || '-';
          ...
          >
            <template #bodyCell="{ column, record }">
              ...
              <template v-else-if="column.dataIndex === 'type'">
                <Tag :color="getTypeColor(record.type)" class="font-mono text-xs">
                  {{ record.type }}
                </Tag>
              </template>
              ...
              <template v-else-if="column.dataIndex === 'description'">
                <div class="space-y-1">
                  ...
                  <div
                    v-if="
                      record.minimum !== undefined || record.maximum !== undefined
                    "
