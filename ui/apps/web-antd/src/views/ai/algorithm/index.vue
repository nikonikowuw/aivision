...
const statsSummary = computed(() => {
  const totalCount = total.value || rawList.value.length;
  let activeCount = 0;
  let builtinCount = 0;
  let customCount = 0;
...
function getActiveFPSTiers(algo: AlgorithmApi.AlgorithmItem) {
  ...
  if (activeVer && Array.isArray(activeVer.fpsTiers) && activeVer.fpsTiers.length > 0) {
  ...
  <Page auto-content-height>
    <div class="space-y-4 pb-6">
      ...
      <div class="grid grid-cols-2 gap-3 sm:grid-cols-4 sm:gap-4">
        ...
        >
          ...
          <p class="mt-1 text-[11px] text-muted-foreground">
            {{ statsSummary.builtinCount }} {{ $t('ai.algorithm.statsBuiltin') }} / {{ statsSummary.customCount }} {{ $t('ai.algorithm.statsCustom') }}
          </p>
        </div>
        ...
        >
          ...
          <p class="mt-1 text-[11px] text-muted-foreground flex items-center gap-1">
      ...
      <div v-else-if="displayList.length > 0">
        <div
          class="grid grid-cols-[repeat(auto-fill,minmax(290px,1fr))] gap-4"
        >
          <div
              ...
              <div>
                ...
                >
                  ...
                  >
                    ...
                    <span>{{ algo.versions?.length || 1 }} {{ $t('ai.algorithm.installedVersions') }}</span>
                  </div>
                </div>
