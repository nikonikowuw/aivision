...
import type { FaceCandidateItem } from './FaceCandidatesTable.vue';
    ...
    <div v-else class="flex flex-col gap-4">
      ...
      >
        ...
        <div class="flex items-center gap-3.5 min-w-0">
          ...
          >
            <IconifyIcon
              :icon="getTargetIcon(capture.targetType)"
              class="size-6"
            />
          </div>
          ...
          <div class="flex flex-col min-w-0 gap-0.5">
            ...
            <div
              class="flex items-center gap-3 text-xs text-muted-foreground truncate flex-wrap"
            >
        ...
        <div class="flex items-center gap-3 shrink-0 ml-auto">
          <div
            class="flex flex-col items-center justify-center rounded-lg border border-border/80 bg-muted/40 px-3 py-1.5 min-w-[80px]"
          >
            <span
              class="text-[10px] uppercase font-semibold text-muted-foreground"
              >{{ $t('record.capture.drawer.confidence') }}</span>
            <Tag
              :color="getConfidenceTagColor(capture.confidence)"
              class="m-0 mt-0.5 font-mono font-bold text-xs"
            >
          ...
          >
            <span
              class="text-[10px] uppercase font-semibold text-muted-foreground"
              >{{ $t('record.capture.drawer.quality') }}</span>
            <Tag
              :color="getConfidenceTagColor(capture.qualityScore)"
              class="m-0 mt-0.5 font-mono font-bold text-xs"
            >
      ...
      >
          ...
          <template #title>
            <div class="flex items-center gap-2 text-xs font-semibold">
              ...
              <span class="text-[11px] font-normal text-muted-foreground">({{
                  $t('record.capture.drawer.panoramaLocation') || '全景定位'
                }})</span>
            </div>
          </template>
          <div
            class="flex items-center justify-center rounded-lg bg-neutral-950 p-2 min-h-[220px]"
          >
            <CaptureThumbnail
        ...
        <Card
          v-if="hasSubCrop"
          size="small"
          class="overflow-hidden shadow-xs border-border/80"
        >
          <template #title>
            <div class="flex items-center gap-2 text-xs font-semibold">
              <IconifyIcon
      ...
      <Card size="small" class="shadow-xs border-border/80">
        <template #title>
          <div class="flex items-center gap-2 text-xs font-semibold">
            ...
            <span>{{
              $t('record.capture.drawer.trackInfo') || '事件与抓拍参数'
            }}</span>
          </div>
        </template>

        <div
          class="grid grid-cols-1 gap-x-6 gap-y-3 sm:grid-cols-2 lg:grid-cols-3 text-xs"
        >
          <!-- 抓拍摄像头 -->
          <div class="flex items-center">
            <span class="meta-label">{{
              $t('record.capture.drawer.camera')
            }}</span>
            <span class="meta-colon">：</span>
            <span
              class="font-medium text-foreground break-all truncate flex-1"
              >{{ capture?.cameraName || capture?.cameraId || '-' }}</span>
          </div>

          <!-- 抓拍时间 -->
          <div class="flex items-center">
            <span class="meta-label">{{
              $t('record.capture.drawer.capturedAt')
            }}</span>
            <span class="meta-colon">：</span>
            <span class="font-mono text-foreground flex-1">{{
              formatDateTime(capture?.capturedAt) || '-'
            }}</span>
          </div>
          ...
          <div class="flex items-center">
            <span class="meta-label">{{
              $t('record.capture.drawer.trackId')
            }}</span>
            <span class="meta-colon">：</span>
            <div class="flex-1">
              <Tag
...
#{{ capture?.trackId }}
          ...
          <div class="flex items-center">
            <span class="meta-label">{{
              $t('record.capture.drawer.algorithm')
            }}</span>
            <span class="meta-colon">：</span>
            <span class="font-mono text-foreground flex-1">{{ capture?.algorithmId }}
              <span class="text-muted-foreground">v{{ capture?.algorithmVersion }}</span></span>
          </div>

          <!-- 事件标识 (带一键复制) -->
          <div class="flex items-center sm:col-span-2 lg:col-span-2">
            <span class="meta-label">{{
              $t('record.capture.drawer.eventId')
            }}</span>
            <span class="meta-colon">：</span>
            <div class="flex items-center gap-1.5 min-w-0 flex-1">
              <span
                class="font-mono text-[11px] truncate text-foreground/90"
                :title="capture?.eventId"
              >
                {{ capture?.eventId }}
              </span>
              <Tooltip :title="$t('system.common.copy') || '复制标识'">
                <Button
                  ...
                  @click="
                    copyToClipboard(
                      capture?.eventId || '',
                      $t('record.face.detail.copySuccess') || '复制成功',
                    )
                  "
                >
      ...
      >
        <template #title>
          <div class="flex items-center gap-2 text-xs font-semibold">
            ...
            <Tag color="blue" class="m-0 text-[10px] font-mono">
Top-{{ faceCandidates.length }}
</Tag>
          </div>
        </template>
        <FaceCandidatesTable
      ...
      >
        ...
        <div class="grid grid-cols-1 gap-2.5 sm:grid-cols-2 lg:grid-cols-3">
          ...
          >
            ...
            <span
              class="font-mono font-semibold text-foreground break-all text-right"
            >
